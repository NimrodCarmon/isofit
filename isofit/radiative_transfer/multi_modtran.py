...
class MultiModtranRT(TabularRT):
    ...
    def __init__(self, engine_config: RadiativeTransferEngineConfig, full_config: Config):
        ...
        self.modtran_lut_names = ['wl', 't_down_dir', 't_down_dif', 't_up_dir', 't_up_dif', 's_albedo', 'rho_atm', 'transms']
    ...
    def load_multi_chn(self, infile, coszen):
...
=======
             ...
             transm  - diffuse and direct irradiance along the
                          sun-ground-sensor path
             transup - transmission along the ground-sensor path only
             trans_dir - direct upwelling tranmittence
             trans_dif - diffuse upwelling tranmittence
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
             Be careful with these! They are to be used only by the
             modtran_tir functions because MODTRAN must be run with a
             reflectivity of 1 for them to be used in the RTM defined
...
>>>>>>> test
        ...
        with open(infile) as f:
            sols, rhoatms, transups, wls, sphalbs, grnd_rflts_1, drct_rflts_1, \
                grnd_rflts_2, drct_rflts_2, transm_dir, transm_dif, L_p_rho_1, widths, \
                    thermal_upwellings, thermal_downwellings, transms, total_rad_0, lp_0, lp_1, lp_2 = \
                    [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], [], []
            print(infile)
            lines = f.readlines()
            for i, line in enumerate(lines):
                ...
                if (i >= 5 and i <= 429):
                    ...
                    rdnatm = float(toks[4]) * 1e6  # uW/nm/sr/cm2 Lp(0) in Guanter
                    ...
                    path_Rad = float(toks[14]) * 1e6 + float(toks[15]) * 1e6 
                    ...
                    lp_0.append(path_Rad)
                    total_rad_0.append(rdnatm) 
                    transms.append(transm)
                    transm_dir.append(A_coeff)
                    transm_dif.append(B_coeff)
                ...
                elif (i >= 435 and i <= 859):
                    line = re.sub('\*+', ' 0.123', line)
                    toks = line.strip().split(' ')
                    toks = re.findall(r"[\S]+", line.strip())
                    wl, wid = float(toks[0]), float(toks[8])
                    grnd_rflt = float(toks[16]) * 1e6
                    drct_rflt = float(toks[17])* 1e6
                    path_Rad = float(toks[14]) * 1e6 + float(toks[15]) * 1e6 
                    
                    L_p_rho_1.append(path_Rad)
                    grnd_rflts_1.append(grnd_rflt)
                    drct_rflts_1.append(drct_rflt)
                    lp_1.append(path_Rad)
                
                elif (i >= 865 and i <= 1289):
                # two 
                    #pdb.set_trace()
                    #line = line.replace("*", "0.123", 1)
                    line = re.sub('\*+', ' 0.123', line)
                    toks = line.strip().split(' ')
                    toks = re.findall(r"[\S]+", line.strip())
                    wl, wid = float(toks[0]), float(toks[8])
                    ...
                    lp_2.append(path_Rad)
                else:
                    continue 

        params = [np.array(i) for i in [wls, widths, sols, rhoatms, transms, sphalbs,
                                        transups, transm_dir, transm_dif,
                                        thermal_upwellings, thermal_downwellings, grnd_rflts_1, 
                                       drct_rflts_1, grnd_rflts_2, drct_rflts_2, L_p_rho_1, total_rad_0, lp_0, lp_1, lp_2]]
        return tuple(params)

    
    def get_atmospheric_terms(self, infile, coszen):
        # currently using the 2-albedo approach, partly based on Guanter 2009
        wvl, wid, sols, rhoatms, transms, sphalbs, transups, transm_dir, transm_dif, \
        thermal_upwellings, thermal_downwellings, grnd_rflts_1, \
        drct_rflts_1, grnd_rflts_2, drct_rflts_2, \
        L_path_1, total_rad_0, Lp0, Lp1, Lp2 = self.load_multi_chn(infile, coszen)
        
        tdir_up = transups
        lg_dir_1 = drct_rflts_1
        lg_dd_1 = grnd_rflts_1
        lg_dir_2 = drct_rflts_2
        lg_dd_2 = grnd_rflts_2
        #lp00 = total_rad_0
        lp_1 = Lp1
        lp_2 = Lp2
        ...
        #sab = sphalbs
        #plt.plot(wvl, lg_dir_1)
        #plt.plot(wvl, lg_dd_1)
        #wvl, t_rad1, rhoatm1, transmA1, transmB1, sphalb1, tdir_up1, lg_dd_1, lg_dir_1, lp_1, wid, ss1, t_irad = load_chn(m6file01, coszen)
        #wvl, t_rad2, rhoatm2, transmA2, transmB2, sphalb2, tdir_up2, lg_dd_2, lg_dir_2, lp_2, wid, ss2, t_irad = load_chn(m6file05, coszen)
        ...
        edir_1 = lg_dir_1 * np.pi / rfl_1 / tdir_up #/mus no per nm, no per sr
        egl_1 = lg_dd_1 * np.pi / rfl_1 / tdir_up # global flux
        edif_1 = egl_1 - edir_1 #diffuse flux
        
        
        egl_2 = lg_dd_2 * np.pi / rfl_2 / tdir_up
        lp00 = (rfl_2 * lp_1 * egl_2 - rfl_1 * lp_2 * egl_1) / (rfl_2 * egl_2 - rfl_1 * egl_1)
        
        tdif_up =  np.pi * (lp_1 - lp00) / (rfl_1 * egl_1) #- tdir_up # Po Kavur Hakelev
        ...
        edir_t = edir_1/mus
        edif_t = edif_1*(1.-rfl_1*sab)#/(ttot_up* ttot_up))#* ttot_up* ttot_up)
        ...
        t_direct_down = ((edir_t*coszen)/wid/np.pi)/TOA_Irad
        t_diffuse_down = ((edif_tt)/wid/np.pi)/TOA_Irad
        #pdb.set_trace()
        params = [np.array(i) for i in [wvl, sols, t_direct_down, t_diffuse_down, tdir_up, tdif_up, \
        sab, rhoatms, transms, thermal_upwellings, thermal_downwellings]]
        return tuple(params)
    ...
    def multi_modtran_driver(self, overrides):
        ...
        param0 = deepcopy(param[0])
        param0['MODTRANINPUT']['CASE'] = 0
        param0['MODTRANINPUT']['SURFACE']['SURREF']=0
        param1 = deepcopy(param[0])
        param1['MODTRANINPUT']['CASE'] = 1
        param1['MODTRANINPUT']['SURFACE']['SURREF']=0.1
        param2 = deepcopy(param[0]) 
        param2['MODTRANINPUT']['CASE'] = 2
        param2['MODTRANINPUT']['SURFACE']['SURREF']=0.5
        params = [param0, param1, param2]
        #pdb.set_trace()
        return json.dumps({"MODTRAN": params}), param
    ...
    def rebuild_cmd(self, point, fn):
        ...
        modtran_config_str, modtran_config = self.multi_modtran_driver(dict(vals))
    ...
    def load_rt(self, fn):
        ...
        params = self.get_atmospheric_terms(chnfile, coszen)
        #pdb.set_trace()
        ...
        names = ['wl', 'sol', 't_down_dir', 't_down_dif', 't_up_dir', 't_up_dif', 's_albedo', 'rho_atm', 'transms']
    ...
    def _get_L_down_transmitted_vswir(self, x_RT, geom):
        r = self.get(x_RT, geom)
        transm = (r['t_down_dir'] + r['t_down_dif']) * (r['t_up_dif'] + r['t_up_dir'])
        rdn = (self.solar_irr*self.coszen) / np.pi * transm
        return rdn
